...
--%><div class="containerFluid" id="conceptBrowserWindowPivotView">
	<div class="row-fluid">
		<div class="span6">
			<div class="container-fluid">
								...
								<div class="row-fluid">
									<div class="offset1 span11">
										<div class="btn-group pad-around">
											...
											<g:else>
												<button type="button" class="btn" id="btnSelectTerm"
													onclick="processSelectHiveTerm('${conceptProxy.origin}','${conceptProxy.preLabel}','${conceptProxy.URI}','${index}')">
													<g:message code="text.select.term" />
												</button>
											</g:else>
										</div>
									</div>
				...
				<g:if test="${conceptProxy.isTopLevel()}">
					<div class="row-fluid" id="conceptBrowserNarrowerLetters">
						<div class="span10 offset1 well">
							<g:render template="/sparqlQuery/alphabetTable" />
						</div>
					</div>
				</g:if>
				<div class="row-fluid">
		...
		<tbody>
			...
			<g:each in="${conceptProxy.narrower.keySet()}" var="key">
				<tr id="${conceptProxy.narrower.get(key)}"
					onclick="processSelectOfTermAsCurrent('${conceptProxy.narrower.get(key)}', '${index}')"
							...
							<tbody>
								...
								<g:each in="${conceptProxy.broader.keySet()}" var="key">
									<tr id="${conceptProxy.broader.get(key)}"
										onclick="processSelectOfTermAsCurrent('${conceptProxy.broader.get(key)}','${index}')"
							...
							<tbody>
								...
								<g:each in="${conceptProxy.related.keySet()}" var="key">
									<tr id="${conceptProxy.related.get(key)}"
										onclick="processSelectOfTermAsCurrent('${conceptProxy.related.get(key)}','${index}')"
