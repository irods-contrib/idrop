...
public class GridAccountServiceImpl extends AbstractConveyorComponentService
    ...
    public GridAccount addOrUpdateGridAccountBasedOnIRODSAccount(
        ...
        try {
            ...
            gridAccount.setSslNegotiationPolicy(irodsAccount.getClientServerNegotiationPolicy().getSslNegotiationPolicy().name());
    ...
    public synchronized IRODSAccount irodsAccountForGridAccount(
        ...
        try {
            decryptedPassword = cacheEncryptor.decrypt(gridAccount
                    .getPassword());
            IRODSAccount irodsAccount = IRODSAccount.instance(gridAccount.getHost(),
            ...
            clientServerNegotiationPolicy.setSslNegotiationPolicy(translateSslNegotiationToEnum(gridAccount.getSslNegotiationPolicy()));
    ...
    /**
     * FIXME: factor out to proper enum in jargon - mcc Ripe for refactoring
     * once a proper enum is in place for ssl negotiation.
     *
     * @param sslNegotiationPolicyString
     * @return {@link SslNegotiationPolicy} enum value
     * @throws JargonException
     */
    public static ClientServerNegotiationPolicy.SslNegotiationPolicy translateSslNegotiationToEnum(final String sslNegotiationPolicyString) throws JargonException {
        if (sslNegotiationPolicyString == null) {
            throw new IllegalArgumentException("null sslNegotiationPolicyString");
        }
        ClientServerNegotiationPolicy.SslNegotiationPolicy returnPolicy = ClientServerNegotiationPolicy.SslNegotiationPolicy.CS_NEG_DONT_CARE;
        String trimPolicy = sslNegotiationPolicyString.trim();
        if (sslNegotiationPolicyString.isEmpty()) {
            // just use default
        } else if (trimPolicy.equals(ClientServerNegotiationPolicy.SslNegotiationPolicy.CS_NEG_DONT_CARE.toString())) {
            returnPolicy = ClientServerNegotiationPolicy.SslNegotiationPolicy.CS_NEG_DONT_CARE;
        } else if (trimPolicy.equals(ClientServerNegotiationPolicy.SslNegotiationPolicy.CS_NEG_REFUSE.toString())) {
            returnPolicy = ClientServerNegotiationPolicy.SslNegotiationPolicy.CS_NEG_REFUSE;
        } else if (trimPolicy.equals(ClientServerNegotiationPolicy.SslNegotiationPolicy.CS_NEG_REQUIRE.toString())) {
            returnPolicy = ClientServerNegotiationPolicy.SslNegotiationPolicy.CS_NEG_REQUIRE;
        } else if (trimPolicy.equals(ClientServerNegotiationPolicy.SslNegotiationPolicy.NO_NEGOTIATION.toString())) {
            returnPolicy = ClientServerNegotiationPolicy.SslNegotiationPolicy.NO_NEGOTIATION;
        } else {
