...
public class DashboardLayoutService {
	 ...
	 * @return
	 */
	public static TransferDashboardLayout layoutDashboard(
		...
		float totalWidth = 0;
		float totalFiles = transfer.getTransferAttempts().get(0)
				.getTotalFilesCount();
		...
		for (TransferAttempt attempt : transfer.getTransferAttempts()) {
			...
			dashboardAttempt.setPercentWidth(60000 + attempt
					.computeTotalTimeInMillis());

			dashboardAttempts.add(dashboardAttempt);
			totalWidth += dashboardAttempt.getPercentWidth();

			// fix the percents for total relative height of the different file
			// status
			float skipHeight = attempt.getTotalFilesSkippedSoFar() / totalFiles;
			float successHeight = (attempt.getTotalFilesTransferredSoFar() - attempt
					.getTotalFilesSkippedSoFar()) / totalFiles;
			float errorHeight = attempt.getTotalFilesErrorSoFar() / totalFiles;
			...
			dashboardAttempt
					.setPercentHeightTransferred(percentHeightTransferred);
		}

		// total width raw number stuck in percent width, so figure out the real
		// width as a percentage of total
