...
public class IRODSTreeTransferHandler extends TransferHandler {
    ...
    public boolean importData(final TransferSupport ts) {
        ...
        // ransferrable = staging view tree
        // transferrable:file:/test1/home/test1/test/log4j-1.2.15.jar
        ...
        DataFlavor[] transferrableFlavors = transferable
        ...
        if (transferable
                .isDataFlavorSupported(IRODSTreeTransferable.irodsTreeDataFlavor)) {
        ...
        for (DataFlavor flavor : transferrableFlavors) {
            ...
            if (flavor.isFlavorJavaFileListType()) {
                ...
                break;
                    ...
                    .getMimeType()
                    .equals("application/x-java-jvm-local-objectref; class=javax.swing.tree.TreeSelectionModel")) {
    ...
    protected Transferable createTransferable(final JComponent c) {
        ...
        try {
            ...
            for (int row : rows) {
                nodesToTransfer.add((IRODSNode) idropGui.getIrodsTree()
                        .getValueAt(row, 0));
            }

            IRODSFileFactory irodsFileFactory = idropGui.getiDropCore()
            ...
            for (IRODSNode nodeToTransfer : nodesToTransfer) {
                ...
                transferFiles.add((File) irodsFileFactory
                        .instanceIRODSFile(objectPath));
            ...
            idropGui.getiDropCore().closeIRODSConnectionForLoggedInAccount();
        }
    ...
    private void processDropOfTreeSelectionModel(
        ...
        CollectionAndDataObjectListingEntry putTarget = (CollectionAndDataObjectListingEntry) parent
                .getUserObject();
        ...
        try {
            ...
            TreePath[] treePaths = transferableSelectionModel
                    .getSelectionPaths();

            for (TreePath treePath : treePaths) {
                LocalFileNode lastPathComponent = (LocalFileNode) treePath
    ...
    private void processPhymoveGesture(final Transferable transferable,
        ...
        try {
            // get the list of files
            sourceFiles = (List<IRODSFile>) transferable
                    .getTransferData(IRODSTreeTransferable.irodsTreeDataFlavor);
        } catch (UnsupportedFlavorException ex) {
            Logger.getLogger(IRODSTree.class.getName()).log(Level.SEVERE, null,
                    ex);
            throw new IdropRuntimeException(
        ...
        moveIRODSFileOrDirectoryDialog
                .setLocation((int) (idropGui.getLocation().getX() + idropGui
                        .getWidth() / 2),
                        (int) (idropGui.getLocation().getY() + idropGui
                        .getHeight() / 2));
    ...
    private void processDropOfFileList(final Transferable transferable,
        ...
        try {
            // get the list of files
            sourceFiles = (List<File>) transferable
