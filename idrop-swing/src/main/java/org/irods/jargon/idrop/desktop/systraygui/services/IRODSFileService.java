...
public class IRODSFileService {
	...
	public List<CollectionAndDataObjectListingEntry> getFilesAndCollectionsUnderParentCollection(
		...
		try {
			CollectionAndDataObjectListAndSearchAO collectionAO = irodsFileSystem
					...
					.getCollectionAndDataObjectListAndSearchAO(irodsAccount);
			return collectionAO
					.listDataObjectsAndCollectionsUnderPath(parentCollectionAbsolutePath);
		} catch (JargonException ex) {
			Logger.getLogger(IRODSFileService.class.getName()).log(
					Level.SEVERE, null, ex);
			throw new IdropException("exception getting collections under: {}"
	...
	public Collection getParentCollection(
		...
		try {
			...
			return collectionAO
					.findByAbsolutePath(parentCollectionAbsolutePath);
		} catch (JargonException ex) {
			Logger.getLogger(IRODSFileService.class.getName()).log(
					Level.SEVERE, null, ex);
			throw new IdropException("exception getting collections under: {}"
	...
	public IRODSRuleExecResult runIRODSRule(final String irodsRule)
		...
		try {
			final RuleProcessingAO ruleProcessingAO = irodsFileSystem
					.getIRODSAccessObjectFactory().getRuleProcessingAO(
							irodsAccount);
	...
	public List<Resource> getResources() throws IdropException {
		...
		try {
			...
			final ResourceAO resourceAO = irodsFileSystem
					.getIRODSAccessObjectFactory().getResourceAO(irodsAccount);
	...
	public List<MetaDataAndDomainData> getMetadataForCollection(
		...
		try {
			final CollectionAO collectionAO = irodsFileSystem
					.getIRODSAccessObjectFactory()
					.getCollectionAO(irodsAccount);
	...
	public List<MetaDataAndDomainData> getMetadataForDataObject(
		...
		try {
			final DataObjectAO dataObjectAO = irodsFileSystem
					.getIRODSAccessObjectFactory()
	...
	public String renameIRODSFileOrDirectory(
		...
		try {
			...
			IRODSFile sourceFile = irodsFileFactory
					.instanceIRODSFile(irodsCurrentAbsolutePath);
