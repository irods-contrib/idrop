...
public class LocalTreeTransferHandler extends TransferHandler {
		...
		if (tree.getHighlightedRow() != closestRow) {
			...
			for (int j = 0; j < tree.getRowCount(); j++) {
				if (closestRow == j) {
					...
					g.fillRect((int) tree.getDirtyRegion().getX(), (int) tree
							.getDirtyRegion().getY(), (int) tree
							.getDirtyRegion().getWidth(), (int) tree
							.getDirtyRegion().getHeight());
	...
	public boolean importData(final TransferSupport ts) {
		...
		if (ts.isDrop()) {
			...
			nodeThatWasDropTarget = (LocalFileNode) targetPath
					.getLastPathComponent();
		} else {
			nodeThatWasDropTarget = (LocalFileNode) tree.getSelectionPath()
					.getLastPathComponent();
		}

		log.info("drop node is: {}", nodeThatWasDropTarget);
		nodeThatWasDropTargetAsFile = (File) nodeThatWasDropTarget
				.getUserObject();
	...
	private void processDropAfterAcceptingDataFlavor(
		...
		if (nodeThatWasDropTargetAsFile.isDirectory()) {
			tempTargetLocalFileAbsolutePath = nodeThatWasDropTargetAsFile
					.getAbsolutePath();
		} else {
	...
	private void processDropFromSerializedObjectType(
		...
		try {
			...
			LocalFileSystemModel fileSystemModel = (LocalFileSystemModel) idropGui
					.getFileTree().getModel();
			log.debug("tree model in transferable:{}", transferableAsTreeModel);

			TreePath[] selectionPaths = transferableAsTreeModel
					.getSelectionPaths();

			File sourceFile;
			LocalFileNode sourceNode;
			for (TreePath selectionPath : selectionPaths) {
				sourceNode = (LocalFileNode) selectionPath
						.getLastPathComponent();
				...
				if (sourceFile.isDirectory()) {
					if (userDropAction == 1) {
						...
						LocalFileNode parentNode = (LocalFileNode) sourceNode
								.getParent();
