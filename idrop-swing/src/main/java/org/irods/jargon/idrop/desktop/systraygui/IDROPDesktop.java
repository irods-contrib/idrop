...
public class IDROPDesktop {
	...
	public void doStartupSequence() {
		...
		try {
			idropCore.setTransferManager(new TransferManagerImpl(idropCore
					.getIrodsFileSystem(), idrop));
		...
		try {
			List<LocalIRODSTransfer> currentQueue = idropCore
					.getTransferManager().getCurrentQueue();
