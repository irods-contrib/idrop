...
public class FinderNewIRODSDirectoryDialog extends javax.swing.JDialog {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3882811106533130534L;
	...
	public FinderNewIRODSDirectoryDialog(final IRODSFinderDialog parent,
			final boolean modal, final String parentDirectory,
			final IRODSFinderTree stagingViewTree, final IRODSNode parentNode) {
		super(parent, modal);
		irodsFinderDialog = parent;
		this.parentDirectory = parentDirectory;
		irodsTree = stagingViewTree;
	...
	private void processNew() {
		...
		if (txtNewFolder.getText().isEmpty()) {
			...
			MessageManager.showError(this, "please enter a new folder name",
			...
			public void run() {
				try {
					...
					try {
						...
						Logger.getLogger(
								FinderNewIRODSDirectoryDialog.class.getName())
					...
					if (!created) {
						log.info("could not create new folder in:{}",
								newDirPath);
						MessageManager.showError(thisDialog,
					...
					try {
						TreePath path = FinderUtils
								.buildTreePathForIrodsAbsolutePath(
										irodsFinderDialog.getIrodsTree(),
										entry.getParentPath());
					...
					MessageManager.showMessage(thisDialog,
							"new folder created", MessageManager.TITLE_MESSAGE);
