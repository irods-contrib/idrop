...
public class QueueSchedulerTimerTask extends TimerTask {
	 ...
	 * final iDrop idropGui) throws IdropException { super();
	 * 
	 * if (transferManager == null) { throw new
	 * IdropException("null transfer manager"); }
	 * 
	 * if (idropGui == null) { throw new IdropException("null idropGui"); }
	 * 
	 * this.transferManager = transferManager; this.idropGui = idropGui;
	 * 
	 * }
	 */
	...
	public void run() {
		...
		/*
		 * if (transferManager.isPaused()) { log.info("timer is paused");
		 * return; }
		 * 
		 * try {
		 * log.info("***** timer queue asking transfer manager to process next"
		 * ); // transferManager.processNextInQueueIfIdle(); } catch
		 * (ConveyorExecutionException ex) {
		 * Logger.getLogger(QueueSchedulerTimerTask.class.getName()).log(
		 * Level.SEVERE, null, ex); idropGui.showIdropException(ex); return; }
		 */
	}
