 ...
 * database is started. This can do any pre-load checks and potentially can
 * clear or detect any necessary migrations/exports before idrop itself starts
 * 
...
public class IdropPreDatabaseBootstrapperServiceImpl implements
	...
	@Override
	public String detectPriorVersion(final String idropHomeDir)
		...
		File versionFile = new File(idropHomeDir,
				IdropConfigurationService.IDROP_VERSION_FILE_NAME);
		if (versionFile.exists()) {
			log.debug("prior version exists...");
			try {
				priorVersion = readFileAsString(versionFile.getAbsolutePath())
						.trim();
			} catch (IOException ex) {
				log.error("error reading prior version file", ex);
				throw new IdropException("Unable to read prior version file",
						ex);
			}
		}
	...
	@Override
	public void storePriorVersion(final String idropHomeDir,
		...
		if (desiredVersionString == null || desiredVersionString.isEmpty()) {
			throw new IllegalArgumentException(
					"null or empty desiredVersionString");
		}
	 ...
	 * @throws IdropException
	 */
	private void reinitializeIdropConfigData(final String idropHomeDir)
			throws IdropException {
		...
		log.info("idropHomeDir:{}...deleting to reinitialize...",
				normalizedHomeDir);
	...
	@Override
	public void triggerMigrations(final String idropHomeDirectory,
		...
		if (idropHomeDirectory == null || idropHomeDirectory.isEmpty()) {
			throw new IllegalArgumentException(
					"idropHomeDirectory is null or empty");
		}
		...
		/**
		 * Right now the only migration is from the point of 3.1 where we
		 * started keeping version numbers
		 */
		if (priorVersion == null
				&& thisVersion.equals(IdropConfigurationService.VERSION_3_1)) {
	...
	private static void deleteChildDirsAndDir(final String absolutePath)
			throws IdropException {
		...
		if (absolutePath.length() <= 6) {
			throw new IllegalArgumentException(
					"path is too short, I shouldn't delete this");
		}
