...
public class LoginDialog extends JDialog {
    ...
    private final iDropLiteCore idropLiteCore;

    public LoginDialog(final JDialog parentDialog, final iDropLiteCore idropLiteCore) {
        super(parentDialog, true);
        this.idropLiteCore = idropLiteCore;
        initComponents();
    ...
    private boolean processLogin() throws NumberFormatException {
        ...
        try {
            ...
            Logger.getLogger(LoginDialog.class.getName()).log(Level.SEVERE,
                    null, ex);
            JOptionPane.showMessageDialog(this, ex.getMessage(), "iDrop Lite Error Message",
        ...
        try {
            irodsFileSystem = idropLiteCore.getIrodsFileSystem();
            AuthResponse authResponse = irodsFileSystem.getIRODSAccessObjectFactory().authenticateIRODSAccount(irodsAccount);
            idropLiteCore.setIrodsAccount(authResponse.getAuthenticatedIRODSAccount());
            this.dispose();
        } catch (JargonException ex) {
